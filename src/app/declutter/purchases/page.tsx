...
import { useAuth } from '@/app/context/AuthContext';
import { getUserPurchases } from '@/app/lib/api';
  ...
  useEffect(() => {
    if (token) {
      getUserPurchases(token)
        .then((data) => {
          setPurchases(data);
          filterPurchases('ongoing', data);
        })
        .catch(console.error);
    }
  }, [token]);

  const filterPurchases = (tab: 'ongoing' | 'completed' | 'refunded', data = purchases) => {
    setActiveTab(tab);
    if (tab === 'ongoing') {
      setFilteredPurchases(data.filter((o) => o.trackingStatus === 'paid' || o.trackingStatus === 'meeting_scheduled'));
    } else if (tab === 'completed') {
      setFilteredPurchases(data.filter((o) => o.trackingStatus === 'completed'));
    } else {
      setFilteredPurchases(data.filter((o) => o.trackingStatus === 'refund_requested' || o.trackingStatus === 'refunded'));
    }
  };

  if (!token) return <div>Please log in to view your purchases.</div>;
    ...
    <div className="min-h-screen bg-gradient-to-br from-black/95 to-gray-900/95 font-sans text-white">
      <nav className="shadow-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center">
          ...
          <h1 className="text-2xl font-semibold ml-4">My Purchases</h1>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div className="flex space-x-4 mb-6">
          <button
            onClick={() => filterPurchases('ongoing')}
          ...
          <button
            onClick={() => filterPurchases('refunded')}
        ...
        <div className="grid gap-6">
          {filteredPurchases.length ? (
            filteredPurchases.map((order) => <OrderCard key={order._id} order={order} />)
          ) : (
            <p className="text-gray-400">No {activeTab} purchases found.</p>
          )}
        </div>
      </main>
    </div>
