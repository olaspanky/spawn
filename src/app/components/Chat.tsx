...
const Chat: React.FC<ChatProps> = ({ room }) => {
  ...
  const socket = useSocket('https://spawnback.vercel.app'); // Replace with your backend URL
  const { user } = useAuth();

  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const response = await fetch(`https://spawnback.vercel.app/api/messages/${room}`);
        if (response.ok) {
  ...
  const sendMessage = async () => {
    if (socket && message.trim() && user) {
      ...
      try {
        const response = await fetch('https://spawnback.vercel.app/api/messages', {
          method: 'POST',
