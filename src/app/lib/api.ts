...
export const scheduleMeeting = (orderId: string, location: string, time: string, token: string) =>
  api.post(`/purchases/${orderId}/schedule`, { location, time }, { headers: { 'x-auth-token': token } }).then(res => res.data);

export const releaseFunds = (orderId: string, token: string) =>
  api.post(`/purchases/${orderId}/release`, {}, { headers: { 'x-auth-token': token } }).then(res => res.data);

export const retractFunds = (orderId: string, reason: string, token: string) =>
  api.post(`/purchases/${orderId}/retract`, { reason }, { headers: { 'x-auth-token': token } }).then(res => res.data);

export const rateSeller = (orderId: string, rating: number, token: string) =>
  api.post(`/purchases/${orderId}/rate`, { rating }, { headers: { 'x-auth-token': token } }).then(res => res.data);